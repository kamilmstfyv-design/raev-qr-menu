...
function Sorbalar() {
  ...
  const sorbalarCategory = menuData.find(item => item.slug === "sorbalar");
  ...
  return (
    ...
    <div className="p-4  mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {sorbalarCategory?.items && sorbalarCategory.items.length > 0 ? (
          sorbalarCategory.items.map((item) => (
            <ProductCard 
              key={item.id} 
              name={item.name} 
              price={item.price} 
